...
def removeBackground(img, hueValue, threshold=0):
    ...
    # threshold_mean or custom threshold
    if threshold == 0:
        th = float(dist.mean())
    else:
        th = threshold
...
def _svm_predict(model, roi):
    ...
    conf = float(np.max(scores) - np.min(scores))
    pred = int(model.predict(feat)[0])
    label = gestureTxt[pred]
    return label, conf
...
def _fuse_prediction(solidity, extent, fingers, svm_label, conf):
    if solidity > SOLIDITY_TH and extent > EXTENT_TH:
        return 'rock'
    rule_label = _rule_predict(fingers)
    if conf >= CONFIDENCE_THRESH or rule_label is None:
        return svm_label
    return rule_label
...
def _draw_overlay(frame, rect, solidity, extent, fingers, label):
    ...
    if rect is not None:
        ...
        cv2.putText(disp, f'sol:{solidity:.2f} ext:{extent:.2f} fin:{fingers}',
                    (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2)
    cv2.putText(disp, f'Gesture: {label}', (10, 30),
